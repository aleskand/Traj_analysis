...
def analyze_trajectory(file, nterminus, top_file=None, nat_knotcore=None, min_gap=10, scope=10, min_knot=100,
                       closure=1, tries=20, max_cross=30, draw_plot=False, plot_filename="knotcore_plot",
    ...
    is a list where the following values are in order: topology type, number of unknotting frame, knot core value in
    frame, where the knot was knotted, number that indicates way of looping, and number that indicates the position
    od the loop.

    e.g. {402: ['3_1', None, (10, 80), 0, 1]}
    e.g. if full_output = True: {402: {'Topology type': '3_1', 'Unknotting frame': None, 'Knot core range': (10, 80),
                                       'Looping via slipknot': True, 'Loop behavior': 'loop is in place'}}
    Symbols used in the code for specific cases:
    - Ways of looping: 0 - knotting via slipknot
                       1 - knotting direct.
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', '--max_cross', type=int, default=30, help='Maximal number of crossings after reduction '
